...
from Anonymous.Core.PyTgCalls.Anonymous import join_live_stream, join_video_stream, stop_stream
from Anonymous.Database import (add_active_chat, is_active_chat, music_off, remove_active_chat, add_active_video_chat,
                            music_on)
from Anonymous.Inline import (audio_markup, audio_markup2, primary_markup, secondary_markup2,
                          secondary_markup)
...
async def start_stream_video(message, file, title, mystic):
    ...
    if await is_active_chat(message.chat.id):
        ...
        final_output = await message.reply_photo(
            ...
            caption=(
                f"ðŸŽ¬<b>__Video:__ </b> [Given Video Via Telegram]({link})\n\nðŸ‘¤<b>__Requested by:__ </b>{message.from_user.mention} \nðŸš§<b>__Queued at:__</b> <b>#{position}!</b>"
            ),
        ...
        await mystic.delete()
        cap = f"ðŸŽ¥<b>__Playing:__ </b>[Given Video Via Telegram]({link})\nðŸ‘¤**__Requested by:__** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
...
async def start_live_stream(
    ...
    cap = f"**Live Streaming**\n\nðŸŽ¥<b>__Playing:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \nðŸ’¡<b>__Info:__</b> [Get Additional Information](https://t.me/{BOT_USERNAME}?start=info_{videoid})\nðŸ‘¤**__Requested by:__** {CallbackQuery.from_user.mention}"
...
async def start_video_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        final_output = await CallbackQuery.message.reply_photo(
            photo=thumb,
            caption=(
                f"ðŸŽ¬<b>Video:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \nâ³<b>__Duration:__</b> {duration_min} \nðŸ’¡<b>__Info:__</b> [Get Additional Information](https://t.me/{BOT_USERNAME}?start=info_{videoid})\nðŸ‘¤<b>__Requested by:__ </b>{CallbackQuery.from_user.mention} \nðŸš§<b>__ Video Queued at:__</b> <b>#{position}!</b>"
            ),
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        await CallbackQuery.message.delete()
