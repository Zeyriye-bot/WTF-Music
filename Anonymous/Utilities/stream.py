...
from Anonymous.Core.PyTgCalls.Anonymous import join_stream
from Anonymous.Database import (add_active_chat, is_active_chat, music_off, add_active_video_chat,
                            music_on)
...
async def start_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        await mystic.delete()
        cap = f"ðŸŽ¥<b>__Playing:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \nðŸ’¡<b>__Info:__</b> [Get Additional Information](https://t.me/{BOT_USERNAME}?start=info_{videoid})\nðŸ‘¤**__Requested by:__** {CallbackQuery.from_user.mention}"
        final_output = await CallbackQuery.message.reply_photo(
            photo=thumb,
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=cap,
...
async def start_stream_audio(
    ...
    if await is_active_chat(message.chat.id):
        ...
        final_output = await message.reply_photo(
            ...
            caption=(
                f"ðŸŽ¬<b>__Audio:__ </b> [Given Audio Via Telegram]({link})\nâ³<b>__Duration:__</b> {duration_min} \nðŸ‘¤<b>__Requested by:__ </b>{message.from_user.mention} \nðŸš§<b>__Queued at:__</b> <b>#{position}!</b>"
            ),
        ...
        await mystic.delete()
        cap = f"ðŸŽ¥<b>__Playing:__ </b>[Given Audio Via Telegram]({link})\nðŸ‘¤**__Requested by:__** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=cap,
